    ...
    uint8_t hasle)
{
    ...
    ret = Se05x_API_Auth_CalculateMacCmdApdu(
        &(session_ctx->scp03_session_mac_Key[0]), &session_ctx->scp03_mcv[0], outBuf, i, macData, &macDataLen);
    ENSURE_OR_RETURN_ON_ERROR((ret == SM_OK), SM_NOT_OK);
        ...
        if (inBufLen > (SCP_COMMAND_MAC_SIZE + SCP_GP_SW_LEN)) { // There is data payload in response
            ...
            ENSURE_OR_RETURN_ON_ERROR(
                (Se05x_API_Auth_GetResponseICV(
                     TRUE, &(session_ctx->scp03_counter[0]), &(session_ctx->scp03_session_enc_Key[0]), iv) == SM_OK),
                SM_NOT_OK);
